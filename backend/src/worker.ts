...
const { Computing, Nft, Storage: Store } = require("@apillon/sdk");
...
const storage = new Store(apillonConfig);

const bucket = storage.bucket("81d863db-cb16-4c3f-8a2c-ca421e307d69");
...
const transfer = async ({
  ...
  const collection = await nft
    ...
    .get();
  const downloadLink = `${process.env.FRONTEND_URL}/#/download?nftId=10`;
